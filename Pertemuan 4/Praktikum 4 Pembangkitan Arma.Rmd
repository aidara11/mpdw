---
title: "Praktikum 4 - Pembangkitan Arma"
...
Akan dicoba membangkitkan proses MA paling sederhana, yaitu MA(2) dengan $\theta = 0.5$ sebanyak 300 observasi dan $c=0$. Karena diperlukan satu nilai awal untuk $e_{t-1}$, masukkan nilai pertama white noise sebagai nilai awal tersebut.
...
Untuk melihat lag ma ada dimana
...
cor(yt_ma,yt_1_ma)
```
Korelasi antara $Y_t$ dengan $Y_{t-1}$ dari hasil simulasi mendekatai perhitungan teoritis.
...
cor(yt_ma2,yt_2_ma)
```
Korelasi antara $Y_t$ dengan $Y_{t-2}$ yaitu 0.4177871.
...
Buat ar lihat lagnya pake plot pacf
```{r}
pacf(ar)
```
...
cor(yt_ar,yt_1_ar)
```
Korelasi antara $Y_t$ dengan $Y_{t-1}$ yaitu $0.6640764$.
...
Korelasi antara $Y_t$ dengan $Y_{t-2}$ yaitu $0.5300309$.
...
Berdasarkan scatterplot tersebut, terlihat bahwa tidak terdapat hubungan positif antara $Y_t$ dengan $Y_{t-3}$.

```{r}
cor(yt_ar3,yt_3_ar)
```
Korelasi antara $Y_t$ dengan $Y_{t-3}$ yaitu $0.4501282$.
...
\begin{bmatrix}
1 \\ 2
\end{bmatrix} & = 0.5 \cdot 1+0.2 \cdot 2\\
&=0.5+0.4 = 0.9
\end{aligned}
$$
...
```{r}
arma22 <- arima.sim(list(order=c(2,0,2), ar = c(0.4,0.6), ma =c(0.5,0.2)), n=300)
arma22
```
